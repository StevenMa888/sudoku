...
export default {
  ...
  methods: {
    ...
    runRuleForGroup: function (rule, group) {
      const vm = this;
      ...
      for (let i = 0; i < 9; i += 3) {
        ...
        for (let k = 1; k <= 9; k++) {
          if (nums[k].length == 2) { // only 1 heng line is missing k
            ...
            let countShu1 = vm.convert(vm.shu[startShu]).countOf(k) | (vm.cells[targetHeng][startShu] != ""); // if this line contains k or the cell is occupied, then it is counted as 1
            let countShu2 = vm.convert(vm.shu[startShu + 1]).countOf(k) | (vm.cells[targetHeng][startShu + 1] != "");
            let countShu3 = vm.convert(vm.shu[startShu + 2]).countOf(k) | (vm.cells[targetHeng][startShu + 2] != "");
      ...
      for (let i = 0; i < 9; i += 3) {
        ...
        for (let k = 1; k <= 9; k++) {
          if (nums[k].length == 2) {
            ...
            let countHeng1 = vm.convert(vm.heng[startHeng]).countOf(k) | (vm.cells[startHeng][targetShu] != ""); // if this line contains k or the cell is occupied, then it is counted as 1
            let countHeng2 = vm.convert(vm.heng[startHeng + 1]).countOf(k) | (vm.cells[startHeng + 1][targetShu] != "");
            let countHeng3 = vm.convert(vm.heng[startHeng + 2]).countOf(k) | (vm.cells[startHeng + 2][targetShu] != "");
            // if both of the other two lines contains k or the position is occupied, then the left target cell can only be k
